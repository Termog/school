	       ...
	       const unsigned int *syn, unsigned int *errloc)
{
	...
	if (!syn) {
		// printf("Generating syndroms\n");
		if (!calc_ecc) {
			// printf("Calculating new ecc\n");
			/* compute received data ecc into an internal buffer */
			if (!data || !recv_ecc) {
				// printf("no data or no recv_ecc\n");
				return -EINVAL;
			}
...
static int build_gf_tables(struct bch_control *bch, unsigned int poly)
{
	...
	printf("Galois field log lookup table\n");
	for (int i = 0; i < (1+bch->n); i++) {
		printf("%d ",bch->a_log_tab[i]);
	}
	printf("\n");
